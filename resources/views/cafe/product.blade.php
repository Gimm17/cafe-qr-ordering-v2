<x-cafe-layout :tableNo="$tableNo" title="{{ $product->name }} - Cafe Order">
    ...
    <div class="max-w-lg mx-auto px-4">
        ...
        <div class="mt-6 mb-6">
            <div class="flex items-center justify-between mb-3">
                <h2 class="font-semibold tracking-tight text-gray-900">Review Pelanggan</h2>
                <div class="flex items-center gap-1">
                    @if($product->average_rating)
                    <svg class="w-4 h-4 text-amber-400" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    <span class="font-bold text-sm text-gray-900">{{ $product->average_rating }}</span>
                    <span class="text-xs text-muted">({{ $reviews->count() }})</span>
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="ui-card-flat p-4">
                    <div class="flex items-center gap-1 mb-1">
                        @for($i=1;$i<=5;$i++)
                            <svg class="w-3.5 h-3.5 {{ $i <= $review->rating ? 'text-amber-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                        @endfor
                        <span class="text-xs text-muted ml-1">{{ $review->created_at->diffForHumans() }}</span>
                    </div>
                    @if($review->comment)
